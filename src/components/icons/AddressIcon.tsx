export const AddressIcon = () => {
  return (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M15 10C15 11.6569 13.6569 13 12 13C10.3431 13 9 11.6569 9 10C9 8.34315 10.3431 7 12 7C13.6569 7 15 8.34315 15 10Z"
        fill="#2E3A59"
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M5 10C5 15.25 12 21 12 21C12 21 19 15.25 19 10C19 6.13 15.87 3 12 3C8.13 3 5 6.13 5 10ZM7 10C7 7.23457 9.23457 5 12 5C14.7654 5 17 7.23457 17 10C17 11.8607 15.6843 14.1296 13.9654 16.1861C13.2714 17.0165 12.5732 17.7402 12 18.2974C11.4268 17.7402 10.7286 17.0165 10.0346 16.1861C8.31569 14.1296 7 11.8607 7 10Z"
        fill="#2E3A59"
      />
    </svg>
  );
};
